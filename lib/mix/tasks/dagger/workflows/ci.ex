defmodule Mix.Tasks.Dagger.Workflows.Ci do
  ...
  def run(opts) do
    ...
    steps = [
      {"Setup", fn -> Mix.Tasks.Dagger.Setup.execute(env: env) end},
      {"Format Check", fn -> Mix.Tasks.Dagger.Format.execute(["--check-formatted"]) end},
      {"Linting", fn -> 
        lint_opts = if skip_dialyzer, do: ["--skip-dialyzer"], else: []
        Mix.Tasks.Dagger.Lint.execute(lint_opts)
      end},
      {"Security Audit", fn -> 
        if skip_security do
          ...
          Mix.Tasks.Dagger.Security.execute([])
        end
      end},
      {"Tests", fn -> Mix.Tasks.Dagger.Test.execute(["--cover"]) end}
    ]
