...
dev-server:
    ...
    @docker compose -f docker-compose.dev.yml run --rm --service-ports app mix phx.server
...
dev-shell:
    ...
    @docker compose -f docker-compose.dev.yml run --rm -it app iex -S mix
...
dev-setup:
    ...
    @docker compose -f docker-compose.dev.yml run -T --rm app mix deps.get
    @echo "Setting up database..."
    @docker compose -f docker-compose.dev.yml run -T --rm app mix ecto.setup
...
db-create:
    ...
    @docker compose -f docker-compose.dev.yml up -d db
    @docker compose -f docker-compose.dev.yml exec -T db sh -c 'until pg_isready -U gatherly; do sleep 1; done'
    @docker compose -f docker-compose.dev.yml run -T --rm app mix ecto.create
...
db-migrate:
    ...
    @docker compose -f docker-compose.dev.yml up -d db
    @docker compose -f docker-compose.dev.yml exec -T db sh -c 'until pg_isready -U gatherly; do sleep 1; done'
    @docker compose -f docker-compose.dev.yml run -T --rm app mix ecto.migrate
...
db-rollback *args="":
    ...
    @docker compose -f docker-compose.dev.yml up -d db
    @docker compose -f docker-compose.dev.yml exec -T db sh -c 'until pg_isready -U gatherly; do sleep 1; done'
    @docker compose -f docker-compose.dev.yml run -T --rm app mix ecto.rollback {{args}}
...
db-reset:
    ...
    @docker compose -f docker-compose.dev.yml up -d db
    @docker compose -f docker-compose.dev.yml exec -T db sh -c 'until pg_isready -U gatherly; do sleep 1; done'
    @docker compose -f docker-compose.dev.yml run -T --rm app mix ecto.reset
...
db-seed:
    ...
    @docker compose -f docker-compose.dev.yml up -d db
    @docker compose -f docker-compose.dev.yml exec -T db sh -c 'until pg_isready -U gatherly; do sleep 1; done'
    @docker compose -f docker-compose.dev.yml run -T --rm app mix run priv/repo/seeds.exs
...
db-shell:
    ...
    @docker compose -f docker-compose.dev.yml up -d db
    @docker compose -f docker-compose.dev.yml exec -T db sh -c 'until pg_isready -U gatherly; do sleep 1; done'
    @docker compose -f docker-compose.dev.yml exec db psql -U gatherly -d gatherly_dev
...
test *args="":
    ...
    @docker compose -f docker-compose.dev.yml exec -T test_db sh -c 'until pg_isready -U gatherly; do sleep 1; done'
    @MIX_ENV=test docker compose -f docker-compose.dev.yml run -T --rm -e MIX_ENV=test app mix test {{args}}
...
format:
    ...
    @docker compose -f docker-compose.dev.yml run -T --rm app mix format

# Run linting
lint:
    @echo "Running linter..."
    @docker compose -f docker-compose.dev.yml run -T --rm app mix credo --strict

# Run Dialyzer type checking
dialyzer:
    @echo "Running Dialyzer..."
    @docker compose -f docker-compose.dev.yml run -T --rm app mix dialyzer
...
deps-update:
    ...
    @docker compose -f docker-compose.dev.yml run -T --rm app mix deps.update --all

# Clean build artifacts
clean:
    @echo "Cleaning build artifacts..."
    @docker compose -f docker-compose.dev.yml run -T --rm app mix clean
    @docker compose -f docker-compose.dev.yml run -T --rm app mix deps.clean --all

# Run arbitrary mix command
mix command:
    @echo "Running mix {{command}}..."
    @docker compose -f docker-compose.dev.yml up -d db test_db
    @docker compose -f docker-compose.dev.yml exec -T db sh -c 'until pg_isready -U gatherly; do sleep 1; done'
...
assets-build:
    ...
    @docker compose -f docker-compose.dev.yml run -T --rm app mix assets.build

# Build and digest assets for production
assets-deploy:
    @echo "Building and digesting assets for production..."
    @docker compose -f docker-compose.dev.yml run -T --rm app mix assets.deploy
